...
async function fetchAndRender() {
  ...
  try {
    // ESPN leaderboard endpoint is broken — use scoreboard instead
    // Convert leaderboard URL to scoreboard URL if needed
    let url = _config.feedUrl;
    if (_config.feedType === 'espn') {
      url = _config.feedUrl.replace('/leaderboard?tournamentId=', '/scoreboard?tournamentId=');
    }
...
function renderGolf() {
  ...
  // Separate active players from cut/WD
  const active = golfField.filter(g => g.status !== 'CUT' && g.status !== 'WD');
  const cut    = golfField.filter(g => g.status === 'CUT' || g.status === 'WD');

  // Cut line: top 50 + ties make the cut at Augusta (standard Masters rule)
  // Use the score of the player at position 50, or last active player if field < 50
  const CUT_POSITION = 50;
  const cutScore = active.length >= CUT_POSITION
    ? active[CUT_POSITION - 1].score
    : active.length > 0 ? active[active.length - 1].score : 999;

  // Near-cut threshold: within 2 shots of the cut line
  const NEAR_CUT_RANGE = 2;
  ...
  // Render active players, inserting cut line at position 50
  active.slice(0, 60).forEach((g, idx) => {
    // Insert cut line before the first player who would miss the cut
    if (!cutLineInserted && g.score > cutScore) {
      cutLineInserted = true;
      html += `
        <div class="cut-line-row">
          <div class="cut-line-label">✂ Projected cut</div>
          <div class="cut-line-bar"></div>
          <div class="cut-line-label">${fmtScore(cutScore)}</div>
        </div>`;
    }

    const picked  = isPicked(g.name);
    const isNear  = !cutLineInserted && g.score >= cutScore - NEAR_CUT_RANGE && g.score <= cutScore;
    const dot     = picked ? '<span class="picked-dot"></span>' : '';
    const thru    = g.thru==='F' ? 'F' : !g.thru||g.thru==='-' ? '—' : 'H'+g.thru;
    const fmt     = n => (n===null||n===undefined) ? '—' : String(n);
    const rowCls  = ['golf-row', picked?'is-picked':'', isNear?'near-cut':''].filter(Boolean).join(' ');
  ...
  // Insert cut line at end of active list if all made the cut (field < 50)
  if (!cutLineInserted && active.length > 0) {
    html += `
      <div class="cut-line-row">
        <div class="cut-line-label">✂ Projected cut</div>
        <div class="cut-line-bar"></div>
  ...
  // Render cut/WD players below the line
  cut.slice(0, 20).forEach(g => {
    ...
    html += `
      <div class="golf-row is-cut${picked?' is-picked':''}">
        ...
        <div class="gcell gcol-r3">${fmt(g.r3)}</div>
        <div class="gcell gcol-r4">${fmt(g.r4)}</div>
        <div class="gcell" style="font-size:12px;color:var(--text-muted)">—</div>
        <div class="gcell cut-label">${g.status === 'WD' ? 'WD' : 'CUT'}</div>
      </div>`;
  });
